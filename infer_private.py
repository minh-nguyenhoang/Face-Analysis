...
def main():
    ...
    test_dataset = TestDataset(root= '/kaggle/input/pixta-preivate-test/private_test_data/private_test_data', json_file= '/kaggle/input/Pixta-preivate-test/private_test/private_test/file_name_to_image_id.json')
    ...
    for idx, batch in enumerate(tqdm(test_dataloader)):
        ...
        images_model = torch.tensor(
            np.array([cv2.cvtColor(cv2.resize(image[int(corner[1]):int(corner[3]), int(corner[0]): int(corner[2])].cpu().numpy(), (224,224)), cv2.COLOR_RGB2BGR) for image, corner in zip(images_, corners)])
            ).to(device)
