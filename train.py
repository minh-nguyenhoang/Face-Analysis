...
def evaluate(model, loss_func, valid_dl, metric=None, device=None, eval=False):
    ...
    with torch.no_grad():
        ...
        if metric is not None:
            ...
            avg_metric = np.sum(np.multiply(torch.stack(torch.tensor(metrics),dim=0).cpu().numpy(), np.array(nums))) / total
...
def trainer(epochs, model, loss_func, train_dl, valid_dl, opt_fn=None, lr=None, metric=accuracy, PATH='', device='cpu'):
    ...
    sched = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=opt, mode='min', patience= 8, min_lr =1e-6, verbose=True)
