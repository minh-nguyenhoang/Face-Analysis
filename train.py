...
def accuracy(outputs, age, gender, masked, emotion, race, skin, eval):
    if len(outputs) == 6:
        ...
        out_age, out_race, out_gender, out_masked, out_emotion, out_skin,_,_,_,_,_,_ = outputs
...
def trainer(epochs, model, loss_func, train_dl, valid_dl, opt_fn=None, lr=None, metric=accuracy, PATH='', device='cpu'):
    ...
    sched = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=opt, mode='min', patience= 12, factor= 0.25, min_lr =1e-6, verbose=True)
