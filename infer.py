...
def main():
    ...
    for idx, batch in enumerate(tqdm(test_dataloader)):
        ...
        image_emo = torch.tensor(
            np.array([cv2.resize(
                image[int(min(image.shape[0], max(0, corner[1] - 0*(corner[3]-corner[1])))): 
                      int(min(image.shape[0], max(0, corner[3] + 0*(corner[3]-corner[1])))), 
                      int(min(image.shape[1], max(0, corner[0] - 0*(corner[2]-corner[0])))):
                      int(min(image.shape[1], max(0, corner[2] + 0*(corner[2]-corner[0]))))].cpu().numpy(), (224,224)) for image, corner in zip(images_, corners)])
            ).to(device)
